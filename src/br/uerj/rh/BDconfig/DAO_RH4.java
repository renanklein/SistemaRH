	...
	//Metodo(s) relacionado(s) ao Caso de Uso RH4
		...
		public static synchronized Funcionario candEfetivado(Candidato efetivado,int id_vaga) {
			...
			try {
				...
				//OBS: Falta atribuir um valor ao atributo processo do func
				novo_func = new Funcionario(mat,efetivado.getCPF(),efetivado.getNome(),efetivado.getProcesso(),true,v.getNum_vaga());
				//Terminou
			}catch(SQLException s) {
				s.getStackTrace();
			}
